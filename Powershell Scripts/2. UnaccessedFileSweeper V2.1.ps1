...
$SendEmail = Read-Host "Send Email Notification? (Y/N)"
$SendTeams = Read-Host "Send Teams Notification? (Y/N)"
...
if ($SendEmail -eq "Y") {
    ...
    $Subject = "File Cleanup Job Completed"
    $Body = $Summary
...
if ($SendTeams -eq "Y") {
    ...
    $TeamsPayload = @{
        ...
        "summary" = "File Cleanup Job Completed"
        "themeColor" = "0076D7"
